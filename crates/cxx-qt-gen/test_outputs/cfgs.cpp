...
template<>
SignalHandler<
  ::rust::cxxqtgen1::QObjectEnabledCxxQtSignalParamsproperty_enabledChanged*>::
  ~SignalHandler() noexcept
{
  if (data[0] == nullptr && data[1] == nullptr) {
    return;
  ...
  drop_QObjectEnabled_signal_handler_property_enabledChanged(
    ::std::move(*this));
}
...
{
  call_QObjectEnabled_signal_handler_property_enabledChanged(*this, self);
}

static_assert(
  alignof(
    SignalHandler<::rust::cxxqtgen1::
                    QObjectEnabledCxxQtSignalParamsproperty_enabledChanged*>) <=
    alignof(::std::size_t),
  "unexpected aligment");
static_assert(
  sizeof(
  ...
  ::Qt::ConnectionType type)
{
  return ::QObject::connect(
    &self,
    &QObjectEnabled::property_enabledChanged,
    &self,
    [&, closure = ::std::move(closure)]() mutable {
